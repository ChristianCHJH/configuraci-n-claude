...
**Credencial (resuelto 2026-08-26):** existe un **PAT clásico llamado `unimar_tms board`** en https://github.com/settings/tokens con scopes `project, read:org, repo` — **vence el 2026-09-06**. También funciona `gh auth refresh -h github.com -s project`: el flujo de dispositivo **sí** deja el scope (el token queda `gist, project, read:org, repo, workflow`), aunque falla al abrir Chrome y hay que pegar https://github.com/login/device a mano. Lo que no sirve es el token que Git Credential Manager guarda para github.com (scopes `gist, repo, workflow`, sin `project`). El device flow de `gh auth login` tampoco: GitHub CLI hoy es GitHub App y devuelve un token `ghu_` sin permisos sobre la org. Hace falta un **PAT clásico con `repo` + `project` + `read:org`**, cargado con `gh auth login --with-token`. `gh` está en `C:\Program Files\GitHub CLI\gh.exe` (instalado 2026-08-07 vía winget).
...
**Estado 2026-08-12: el board tiene 33 items** — las 32 historias del MVP + **US-TMS-043** (issue #64, primera historia **técnica** del backlog: contrato de APIs con SAP; EP-01, Sprint 1, M, Must, En Refinamiento). **IDs de iteración** (`PVTIF_lADOEenn8M4Bfr4ezhZ9DtI`): Sprint 1 `33742a74` (2026-08-07) · **Sprint 2 `815896a5` (2026-08-28)** · Sprint 3 `a90f9d00` (2026-09-18), 21 días cada uno. **IDs de campo:** Status `PVTSSF_lADOEenn8M4Bfr4ezhZ9CPY` · Size `…zhZ9DtA` · Épica `…zhZ9pg0` · Prioridad `…zhZ9ptA`.

**Estado 2026-08-26: el board tiene 44 items.** Entraron las **5 historias técnicas del Sprint 2** — US-TMS-045 (#82, UMS) · 046 (#83, UniSAP REST) · 047 (#84, arquitectura Kotlin) · 048 (#85, modelo de datos) · 049 (#86, prototipo) — todas en Iteration Sprint 2, Status En Refinamiento, asignadas a Christian, con su `.md` en `docs/01-concepcion/stories/` (**las historias se movieron de `_bmad-output/planning-artifacts/` a `docs/01-concepcion/stories/`**). Épica: 046→EP-01, el resto EP-06 (045/047 propias; **048 y 049 son transversales EP-04+EP-06 y se forzaron a EP-06** porque el campo es de valor único).

El campo `Iteration` de Sprint 1 se llama literalmente `Sprint 1:Planificación completa (sin app móvil)` (sin espacio tras los dos puntos), id `33742a74`.
